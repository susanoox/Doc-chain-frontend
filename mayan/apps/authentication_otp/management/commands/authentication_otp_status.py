...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            dest='username', help='Username for which OTP status will be '
            'displayed.', metavar='<username>'
        )
    ...
    def handle(self, username, **options):
        try:
            ...
            status_text = 'Unknown username or OTP data is not initialized '
            'correctly.'
        else:
        ...
        self.stdout.write(
            msg='OTP state is: {}'.format(status_text)
        )
