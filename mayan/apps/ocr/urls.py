...
urlpatterns_document_types = [
    re_path(
        route=r'^document_types/submit/$', name='document_type_submit',
        view=DocumentTypeOCRSubmitView.as_view()
    ),
    re_path(
        route=r'^document_types/(?P<document_type_id>\d+)/settings/$',
...
api_urls = [
    re_path(
        route=r'^document_types/(?P<document_type_id>\d+)/ocr/settings/$',
        name='document-type-ocr-settings-view',
        view=APIDocumentTypeOCRSettingsView.as_view()
    ),
    re_path(
        route=r'^documents/(?P<document_id>\d+)/ocr/submit/$',
        name='document-ocr-submit-view',
        view=APIDocumentOCRSubmitView.as_view()
    ),
    re_path(
        route=r'^documents/(?P<document_id>\d+)/versions/(?P<document_version_id>\d+)/ocr/submit/$',
        name='document-version-ocr-submit-view',
        view=APIDocumentVersionOCRSubmitView.as_view()
    ),
    re_path(
        route=r'^documents/(?P<document_id>\d+)/versions/(?P<document_version_id>\d+)/pages/(?P<document_version_page_id>\d+)/ocr/$',
